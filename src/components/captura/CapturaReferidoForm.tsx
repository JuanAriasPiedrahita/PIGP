...
import type { Atributos, Referido } from "@/lib/types";
...
/**
 * Formulario de referido para la zona de captura de líderes (/captura).
 * A diferencia del formulario del panel de administración, aquí nunca se
 * elige el líder: el backend lo toma de la sesión del líder autenticado.
 */
export function CapturaReferidoForm({ catalogos, referidoId, onSaved, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    apiGet<Referido>(`/api/captura/referidos/${referidoId}`)
      .then((r) => {
        setForm({
          cedula: r.cedula,
          nombre: r.nombre,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (referidoId) {
        await apiPutJson(`/api/captura/referidos/${referidoId}`, payload);
        toast.show("Referido actualizado correctamente", "success");
      } else {
        await apiPostJson("/api/captura/referidos", payload);
