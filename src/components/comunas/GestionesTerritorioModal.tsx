...
import type { TerritorioResumenGestion, TerritorioDetalleGestion, TipoAyudaReferidoResuelta } from "@/lib/types";
...
export function GestionesTerritorioModal({ seleccion, onClose }: Props) {
  ...
  const [tipoAyudaFilas, setTipoAyudaFilas] = useState<TipoAyudaReferidoResuelta[]>([]);
  const [loadingTipoAyuda, setLoadingTipoAyuda] = useState(false);

  const toast = useToast();
  const esTipoAyuda = seleccion?.tipo === "tipo_ayuda";
  ...
  useEffect(() => {
    ...
    if (seleccion.tipo === "tipo_ayuda") {
      setLoadingTipoAyuda(true);
      apiGet<TipoAyudaReferidoResuelta[]>(`/api/gestiones/tipo-ayuda/resueltas?tipo_ayuda_id=${seleccion.id}`)
        .then(setTipoAyudaFilas)
        .catch((err) => toast.show(err instanceof Error ? err.message : "Error cargando las ayudas resueltas", "error"))
        .finally(() => setLoadingTipoAyuda(false));
      return;
    }
  ...
  const titulo = seleccion
    ? esTipoAyuda
      ? `Ayudas resueltas de tipo ${seleccion.nombre}`
      : detalle
  ...
  const totalVeces = tipoAyudaFilas.reduce((sum, f) => sum + Number(f.veces), 0);
  const totalInvertidoTipoAyuda = tipoAyudaFilas.reduce((sum, f) => sum + Number(f.total_invertido), 0);
  ...
  return (
    <Modal open={!!seleccion} onClose={onClose} title={titulo} widthClass="max-w-2xl">
      {esTipoAyuda ? (
        ...
        ) : tipoAyudaFilas.length === 0 ? (
          <p className="py-10 text-center text-sm text-slate-400">No hay ayudas de este tipo resueltas todavía.</p>
        ) : (
            ...
            <table className="w-full min-w-[480px] text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                  ...
                  <th className="px-3 py-2 text-center font-medium">Veces recibida</th>
                  <th className="px-3 py-2 text-right font-medium">Total invertido</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {tipoAyudaFilas.map((f) => (
                  <tr key={f.referido_id}>
                    <td className="py-2 pr-3 font-medium text-slate-800">{f.colaborador}</td>
                    <td className="px-3 py-2 text-center text-slate-600">{f.veces}</td>
                    <td className="px-3 py-2 text-right text-slate-600">{formatCosto(f.total_invertido)}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr className="border-t border-slate-200 font-semibold text-slate-800">
                  ...
                  <td className="px-3 py-2 text-center">{totalVeces}</td>
                  <td className="px-3 py-2 text-right">{formatCosto(totalInvertidoTipoAyuda)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
