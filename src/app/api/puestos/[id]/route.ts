...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await pool.query(
      "UPDATE puestos SET zona_id = ?, numero = ?, nombre = ?, direccion = ?, num_mesas = ? WHERE id = ?",
      [zona_id, numero, nombre, direccion || null, num_mesas || 1, params.id]
    );
    return NextResponse.json({ id: Number(params.id), zona_id, numero, nombre, direccion: direccion || null, num_mesas });
  } catch (err) {
